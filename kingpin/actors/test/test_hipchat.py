...
class TestHipchatBase(testing.AsyncTestCase):
    ...
    def test_validate_from_name(self):
        ...
        room = "Operations"
        actor = hipchat.Message("Unit Test Action", {"message": message, "room": room})
    ...
    def test_init_without_environment_creds(self):
        ...
        with self.assertRaises(exceptions.InvalidCredentials):
            hipchat.Message("Unit Test Action", {"room": "test", "message": "test"})
...
class TestHipchatMessage(testing.AsyncTestCase):
    ...
    def test_execute_dry_mode_response(self):
        ...
        # Valid response test
        response_dict = {
            "success": {"code": 202, "type": "Accepted", "message": "It worked"}
        }
        response_body = json.dumps(response_dict)
        http_response = httpclient.HTTPResponse(
            httpclient.HTTPRequest("/"), code=202, buffer=io.StringIO(response_body)
        )
    ...
    def test_execute_with_401(self):
        ...
        response_body = json.dumps(response_dict)
        http_response = httpclient.HTTPError(code=401, response=response_body)
    ...
    def test_execute_with_403(self):
        ...
        # Valid response test
        response_dict = {
            "error": {"code": 403, "type": "Forbidden", "message": "Hit the rate limit"}
        }
        response_body = json.dumps(response_dict)
    ...
    def test_execute_with_unknown_exception(self):
        ...
        # Valid response test
        response_dict = {
            "error": {"code": 123, "type": "Unknown", "message": "Auth token not found"}
        }
        response_body = json.dumps(response_dict)
...
class TestHipchatTopic(testing.AsyncTestCase):
    ...
    def test_execute(self):
        ...
        room = "unit_room"
        actor = hipchat.Topic("Unit Test Action", {"topic": topic, "room": room})
