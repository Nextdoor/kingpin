...
class MCIBaseActor(base.RightScaleBaseActor):
    ...
    def _check_image_options(self, images_list):
        """TODO: add docstring"""
    ...
    def _get_image_def(self, description):
        ...
        if description.get('user_data'):
            params['user_data'] = description['user_data']
    ...
    def _generate_mci_params(self):
        new_mci_params = self._generate_rightscale_params(
            prefix='multi_cloud_image',
            params={
                'description': self.option('description'),
                'name': self.option('name')
            })
        return new_mci_params

    @gen.coroutine
    def _get_settings_params(self):
        image_futures = []
        for image in self.option('images'):
            image_futures.append(self._get_image_def(image))
        settings_params = yield image_futures

        raise gen.Return(settings_params)

    def _diff_setting(self, current_setting_obj, new_param):
        objects_params = self._setting_to_param(current_setting_obj)
        return set(objects_params) != set(new_param)

    def _get_cloud_from_setting_param(self, setting_param):
        return [s[1] for s in setting_param if 'cloud_href' in s[0]][0]

    def _setting_to_param(self, setting_obj):
        data = setting_obj.links
        return [
            ('multi_cloud_image_setting[image_href]', data['image']),
            # ('multi_cloud_image_setting[user_data]', data[...]),
            ('multi_cloud_image_setting[instance_type_href]', data[
             'instance_type']),
            ('multi_cloud_image_setting[cloud_href]', data['cloud'])]
...
class Create(MCIBaseActor):
    ...
    **Examples**
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._check_image_options(self.option('images'))
    ...
    def _execute(self):
        ...
        if self._dry:
            ...
            for setting in settings_params:
...
class Update(MCIBaseActor):

    """Updates a RightScale Multi Cloud Image.
    ...
    **Options**
    ...
    :name:
      The name of the MCI to be updated.

    :description:
       The description of the MCI to be updated.
       (*optional*)
    ...
    **Examples**
    ...
    @gen.coroutine
    def _ensure_setting(self, mci, param, old_settings):
        # Create a mapping between clouds and settings
        old_cloud_hrefs = dict((s.links['cloud'], s) for s in old_settings)

        this_cloud = self._get_cloud_from_setting_param(param)

        if this_cloud in old_cloud_hrefs:
            old_setting_obj = old_cloud_hrefs[this_cloud]
            if self._diff_setting(old_setting_obj, param):
                self.log.warning('Updating setting for image...')
                yield self._client.update(old_setting_obj, param)
            else:
                self.log.warning('No need to update this bitch')
        else:
            self.log.warning('Creating new setting for image...')
            yield self._client.create_resource(mci.settings, param)

    @gen.coroutine
    def _execute(self):

        mci = yield self._find_mci()

        new_mci_params = self._generate_mci_params()

        settings_params = yield self._get_settings_params()

        old_settings = yield self._client.show(mci.settings)

        self.log.info('Updating MCI %s' % self.option('name'))
        yield self._client.update(mci, new_mci_params)

        for param in settings_params:
            yield self._ensure_setting(mci, param, old_settings)

        # Collect a list of cloud hrefs. /api/clouds/3 etc...
        new_cloud_hrefs = [
            img['multi_cloud_image_setting[cloud_href]']
            for img in [dict(img) for img in settings_params]]

        for setting in old_settings:
            cloud = setting.links['cloud']
            if cloud not in new_cloud_hrefs:
                self.log.info('Deleting %s.' % cloud)
                yield self._client.destroy_resource(setting)
